# Prototipo visual de SIA

Mockup navegable de la vista de Coordinadora y un proyecto. Usa datos ficticios y no se conecta a autenticación, base de datos ni servicios externos.

## Ejecutar localmente

```bash
pnpm dev
```

## Construir

```bash
pnpm build
```

## Archivo autónomo

`bundle.html` contiene una versión autocontenida para abrir en un navegador sin ejecutar el proyecto.

## Alcance actual

- Dashboard de Coordinadora.
- Navegación simulada de un proyecto.
- Resumen, objetivos y actividades, reuniones, trámites, presupuesto, documentos y entregables, chat, alertas y equipo.
- Paleta institucional y marca SIA.
